...
class SplashUpdater:
    def __init__(self):
        ...
        self.lbl_status = tk.Label(self.root, text="正在连接云端检查更新...", font=("Microsoft YaHei", 9), fg="gray")
    ...
    def check_update_logic(self):
        try:
            ...
            # 联网失败，提示并直接进入游戏
            self.root.after(0, lambda: self.skip_update(f"网络连接失败 ({str(e)})，将直接进入游戏。"))
    ...
    def download_logic(self):
        try:
            ...
            if os.path.exists(current_file): 
                ...
                except: os.rename(current_file, current_file + ".old")
    ...
    def skip_update(self, reason):
        ...
        self.root.after(1000, self.finish_and_launch)
...
def run_dedicated_server():
    ...
    def handle_client(client_conn, client_id):
        ...
        while True:
            try:
                data = client_conn.recv(2048).decode()
                if not data: break
                buffer += data
                while "\n" in buffer:
...
def run_ursina_game(multiplayer_mode=False):
    import_ursina()  # 真正进入游戏进程时才引入 3D 引擎资源
    
    net_status = {"socket": None, "connected": False}
    other_players = {}
    msg_queue = Queue() # 换用线程安全队列，杜绝多线程包冲突引起的闪退
    ...
    Text(parent=lobby_panel, text="提示: [Ctrl+5] 大厅 | [1-5] 切换物品 | 连按 10 次 [空格] 存档退出", scale=1, y=-0.4, origin=(0, 0), color=color.light_gray)
...
def create_main_launcher():
    ...
    root.geometry(f"+{int((sw-320)/2)}+{int((sh-340)/2)}")
    ...
    tk.Button(root, text="作者信息", command=lambda: messagebox.showinfo("关于作者", "本程序由 mojang AB·杜奕洲·notch·Gemini共同制作\n感谢您的使用"), font=("Microsoft YaHei", 10)).pack(fill="x", padx=20, pady=3)
    tk.Button(root, text="访问官网", command=lambda: webbrowser.open("http://minecraft.net"), font=("Microsoft YaHei", 10)).pack(fill="x", padx=20, pady=3)   
    tk.Button(root, text="更多内容", command=lambda: messagebox.showinfo("more", f"当前版本: V{CURRENT_VERSION}\n期待后续更新......"), font=("Microsoft YaHei", 10)).pack(fill="x", padx=20, pady=3)   
