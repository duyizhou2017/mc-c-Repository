...
class SplashUpdater:
    ...
    def check_update_logic(self):
        try:
            ...
            self.root.after(0, lambda: self.skip_update(f"网络连接失败，已跳过更新步骤。"))
...
def run_dedicated_server():
    ...
    try:
        server_socket.bind(("0.0.0.0", 25565)) # 支持局域网内其他设备连接
...
def run_ursina_game(multiplayer_mode=False):
    ...
    server_list = [
        {"name": "本地回环测试服", "ip": "127.0.0.1", "port": 25565},
    ]
