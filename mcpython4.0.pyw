...
def import_ursina():
    ...
    try:
        ...
        # 如果没有该库，后台尝试自动静默安装
        os.system(f"{sys.executable} -m pip install perlin-noise")
        from perlin_noise import PerlinNoise
...
def run_ursina_game(multiplayer_mode=False):
    ...
    # === 【核心修复】移除灰蒙蒙的视觉迷雾，开阔视野 ===
    camera.clip_plane_far = 250         # 将摄像机渲染最远剪裁距离大幅抬高
    scene.fog_density = 0.003           # 极低雾气密度，让远景可见
    scene.fog_color = color.light_gray   # 雾气主色调调柔和
            ...
            if self.hovered and not lobby_panel.enabled: 
                if key == 'right mouse down':
                    ...
                    if multiplayer_mode and net_status["connected"]: 
                        ...
                        v = Voxel(position=pos, is_manual=True)
                        generated_chunks[(int(pos.x), int(pos.y), int(pos.z))] = v
                if key == 'left mouse down':
                    if multiplayer_mode and net_status["connected"]: 
                        try: net_status["socket"].sendall((json.dumps({"type": "break_block", "x": self.position.x, "y": self.position.y, "z": self.position.z}) + "\n").encode('utf-8'))
                        except: pass
                    else: 
                        pos_key = (int(self.position.x), int(self.position.y), int(self.position.z))
    ...
    if isinstance(saved_blocks, list) and saved_blocks:
        for b in saved_blocks:
            try:
                v = Voxel(position=(b['x'], b['y'], b['z']), custom_color=color.rgb(b['c'][0], b['c'][1], b['c'][2]), is_manual=True)
                generated_chunks[(b['x'], b['y'], b['z'])] = v
            except: pass

    # === 【核心修复】为了防止玩家刚出生看到灰白虚空，提前在脚下预生成地表区块 ===
    for x in range(-12, 12):
        for z in range(-12, 12):
            noise_val = noise([x / 30, z / 30])
            y = int(noise_val * 8) + 8
            if (x, y, z) not in generated_chunks:
                if y > 11: chosen_color = BLOCK_TYPES[2]["color"]
                elif y < 6: chosen_color = BLOCK_TYPES[1]["color"]
                else: chosen_color = BLOCK_TYPES[0]["color"]
                generated_chunks[(x, y, z)] = Voxel(position=(x, y, z), custom_color=chosen_color)
    ...
    def handle_infinite_terrain():
        ...
        for pos_key in list(generated_chunks.keys()):
            ...
            if not generated_chunks[pos_key].is_manual:
                if abs(bx - px) > RENDER_DISTANCE + 4 or abs(bz - pz) > RENDER_DISTANCE + 4:
                    destroy(generated_chunks[pos_key])
                    del generated_chunks[pos_key]

    # 初始化第一人称角色（安全高度 Y=20 避免穿模夹空）
    ...
    def update():
        ...
        if player.y < -30 and not player.is_dead: 
            player.is_dead = True
            death_text.enable()  
...
def create_main_launcher():
    ...
    tk.Button(root, text="扩展与未来规划", command=lambda: messagebox.showinfo("3.1 稳定版更新", f"1. 修复了进入游戏视野内一片灰蒙蒙的视觉Bug。\n2. 优化了预生成逻辑，防止由于主线程卡顿导致坠落虚空。"), font=("Microsoft YaHei", 9)).pack(fill="x", padx=20, pady=2)   
